...
fn is_nullcoalesce(expr: &aast::Expr<(), ()>) -> bool {
    expr.2
        .as_binop()
        .is_some_and(|binop| matches!(binop.bop, Bop::QuestionQuestion))
}

fn is_nullcoalesce_or_bitwise(expr: &aast::Expr<(), ()>) -> bool {
    expr.2.as_binop().is_some_and(|binop| {
        matches!(
            binop.bop,
            Bop::QuestionQuestion | Bop::Amp | Bop::Bar | Bop::Xor
        )
    })
}
...
fn expand_trivial_expr_conds(
    ...
    let needs_parens = is_nullcoalesce_or_bitwise(expr);
...
pub(crate) fn check_implicit_boolean_conversion(
    ...
    let negation_end_offset = pos.start_offset();

    let mut bool_cast_offsets = None;
    if let Some(cast_expr) = expr.2.as_cast()
        && let Some((hint_id, _)) = cast_expr.0.as_happly()
        && hint_id.name() == "bool"
    {
        bool_cast_offsets = Some((
            hint_id.pos().start_offset() as u32 - 1,
            hint_id.pos().end_offset() as u32 + 1,
        ));
        expr = cast_expr.1;
        pos = expr.pos();
    }
    ...
    if !expr_type.is_bool() {
            ...
            && let Some(migration) = TRUTHINESS_MIGRATIONS.iter().find(|m| m.matches(&expr_type))
        {
            ...
            if statements_analyzer.should_autofix(context, analysis_data, &issue) {
                if negation_depth > 0
                    && let Some((_, bool_cast_end_offset)) = bool_cast_offsets
                {
                    // The negation and bool-cast removals can be adjacent, which the
                    // replacement collector treats as overlapping. Remove them together,
                    // preserving any grouping parentheses between them.
                    let mut prefix = statements_analyzer.file_analyzer.file_source.file_contents
                        [negation_start_offset..bool_cast_end_offset as usize]
                        .to_string();
                    let bool_cast_start_offset = bool_cast_offsets.unwrap().0 as usize;
                    prefix.replace_range(bool_cast_start_offset - negation_start_offset.., "");
                    analysis_data.add_replacement(
                        (negation_start_offset as u32, bool_cast_end_offset),
                        Replacement::Substitute(prefix.replace("!", "")),
                    );
                } else if negation_depth > 0 {
                    ...
                    let negations = &statements_analyzer.file_analyzer.file_source.file_contents
                        [negation_start_offset..negation_end_offset];
                    analysis_data.add_replacement(
                        (negation_start_offset as u32, negation_end_offset as u32),
                        Replacement::Substitute(negations.replace("!", "")),
                    );
                } else if let Some((start, end)) = bool_cast_offsets {
                    analysis_data.add_replacement((start, end), Replacement::Remove);
                }
