...
pub(crate) fn analyze(
    ...
    if let Some(keyed_array_var_id) = &keyed_array_var_id {
        if context.has_variable(keyed_array_var_id) {
            ...
            context
                .locals
                .insert(VarName::new(keyed_array_var_id.clone()), stmt_type.clone());
        ...
        if let Some(keyed_array_var_id) = &keyed_array_var_id {
            ...
            if !context.inside_isset && can_store_result && keyed_array_var_id.contains("[$") {
                context.locals.insert(
                    VarName::new(keyed_array_var_id.clone()),
                    Rc::new(stmt_type_inner.clone()),
                );
            }
        }
            ...
            analysis_data.maybe_add_issue(
                Issue::new(
                    ...
                    format!(
                        ...
                        array_type.get_id(Some(statements_analyzer.interner)),
                        offset_type.get_id(Some(statements_analyzer.interner))
                    ),
                    statements_analyzer.get_hpos(stmt.2),
                    &context.function_context.calling_functionlike_id,
                ),
                    ...
                    analysis_data.maybe_add_issue(
                        Issue::new(
                            ...
                            format!(
                                ...
                                vec.get_id(Some(statements_analyzer.interner)),
                                val
                            ),
            ...
            analysis_data.maybe_add_issue(
                Issue::new(
                    ...
                    format!(
                        ...
                        mixed.get_id(Some(statements_analyzer.interner))
                    ),
                    statements_analyzer.get_hpos(pos),
                    &context.function_context.calling_functionlike_id,
                ),
