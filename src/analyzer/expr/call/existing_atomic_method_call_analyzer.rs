        ...
        StrId::PUT => {
            ...
            if call_expr.1.len() == 3 {
                    ...
                    (expr_type, dim_var_id, value_type)
                {
                    ...
                    // Set up dataflow to match the behavior of:
                    //   $result = $s;
                    //   $result['y'] = $value;
                    //
                    // 1. Create a result node
                    // 2. Old shape data flows through with PathKind::Default
                    // 3. New value flows through with PathKind::ArrayAssignment
                    //
                    // This way, when fetching $result['y'], only the value matches
                    // (via ArrayAssignment/ArrayFetch pairing), while old shape data
                    // flows through but won't match a specific key fetch.
